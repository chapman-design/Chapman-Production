...
// The user wants the text to wrap around the image rather than sitting in a rigid 50/50 flex column split.
// The easiest way to accomplish true text wrapping in a standard section is to float the image.

// Let's modify the StandardSection layout specifically to support text wrapping if a special flag is passed, or just change the default desktop layout entirely for all standard sections to float the image instead of using flex box.
// Given the simplicity, let's change the layout to use a floated image on desktop, while remaining stacked on mobile.

const target = `<div className={\`w-full \${section.image ? 'lg:w-1/2' : 'max-w-4xl'} space-y-6\`}>`;
const replacement = `<div className="w-full space-y-6">`;

// We'll rewrite the entire StandardSection return block to support floating images
