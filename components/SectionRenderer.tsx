...
const StandardSection: React.FC<{ section: Section; idx: number; logoUrl?: string; siteName?: string; hasScrolled: boolean }> = ({ section, idx, logoUrl, siteName, hasScrolled }) => {
      ...
      <div className="w-full">
        {section.image && (
          <motion.div className={`w-full lg:w-1/2 lg:mb-6 overflow-hidden shrink-0 ${isLeft ? 'lg:float-left lg:mr-12' : 'lg:float-right lg:ml-12'} mb-8`} viewport={{ once: false, amount: 0.35, margin: '-10% 0px -10% 0px' }} onViewportEnter={() => setIsInView(true)} onViewportLeave={() => setIsInView(false)}>
            <motion.img whileHover={{ scale: 1.05 }} transition={{ duration: 1.2, ease: [0.33, 1, 0.68, 1] }} src={getImageUrl(section.image)} alt={section.imageCaption || section.title || 'Design project'} className={`w-full aspect-[4/3] lg:aspect-auto lg:h-[600px] object-cover shadow-sm hybrid-bloom-image ${shouldBloom ? 'in-view' : ''}`} loading="lazy" referrerPolicy="no-referrer" />
          </motion.div>
        )}
        {isHeritage && logoUrl && (
          <div className="mb-6 flex justify-start">
            <img src={logoUrl} alt={siteName || 'Chapman Design Associates Logo'} className="h-32 md:h-48 w-auto object-contain select-none mix-blend-multiply" referrerPolicy="no-referrer" />
          </div>
        )}
        {section.title && <h2 className="text-3xl font-bold text-stone-900 tracking-tight mb-6">{section.title}</h2>}
        <div className={`text-stone-800 leading-relaxed text-lg font-light prose prose-stone max-w-none ${!isExpanded && section.content && section.content.length > 600 ? 'line-clamp-6' : ''}`}>
